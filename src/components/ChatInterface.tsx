        ...
        <div className={styles.headerContent}>
          <div className={styles.headerInner}>
            <div className={styles.headerLeft}>
              <div
                className={`${styles.agentAvatar} ${styles[getAgentColorClass(selectedAgent)]}`}
              >
        ...
        <div className={styles.messagesContent}>
          {messages.length === 0 && (
            <div className={styles.emptyState}>
              ...
              <h3 className={styles.emptyTitle}>Ready to help!</h3>
              <p className={styles.emptyDescription}>
                Start a conversation with your selected AI agent. Ask questions,
                get insights, or just chat!
              </p>
            </div>
          )}
          ...
          {messages.map(message => (
            ...
            >
              ...
              {/* Message Content */}
              <div
                className={`${styles.messageContent} ${message.role === 'user' ? styles.user : ''}`}
              >
                <div
                      ...
                      part.type === 'text' ? (
                        <ReactMarkdown
                          ...
                          components={{
                            ...
                            ol: ({ children }) => (
                              <ol className={styles.markdownOrderedList}>
                                {children}
                              </ol>
                            ),
                            li: ({ children }) => (
                            ...
                            strong: ({ children }) => (
                              <strong className={styles.markdownStrong}>
                                {children}
                              </strong>
                            ),
                            em: ({ children }) => (
                ...
                <p
                  className={`${styles.messageTime} ${message.role === 'user' ? styles.user : ''}`}
                >
                  {formatTime(new Date())}
                </p>
              </div>
          ...
          {isLoading && (
            <div className={styles.loadingMessage}>
              <div
                className={`${styles.loadingAvatar} ${styles[getAgentColorClass(selectedAgent)]}`}
              >
                <span>{getAgentIcon(selectedAgent)}</span>
              </div>
